...
KeyOp::KeyOp() : last_zero_vel_sent_(true), // avoid zero-vel messages from the beginning
                 ...
                 linear_vel_step_(0.01),
                 linear_vel_max_(2.0),
                 angular_vel_step_(3.0),
                 angular_vel_max_(605),
                 location_step_(100),
                 up_down_location_(1000),
                 max_location_(42396),
                 min_location_(1000),
...
bool KeyOp::init()
{
   ...
   ** Publishers
   **********************/
  velocity_publisher_ = nh.advertise<geometry_msgs::Twist>("/qfeel/cmd_vel", 1);
  enable_motors_publisher_ = nh.advertise<std_msgs::String>("/qfeel/enable_motors", 1);
  disable_motors_publisher_ = nh.advertise<std_msgs::String>("/qfeel/disable_motors", 1);
  up_down_publisher_ = nh.advertise<qfeel_keyop::location>("/qfeel/up_down", 1);
  ...
  cmd_->angular.y = 0.0;
  cmd_->angular.z = 0.0;
  location_.up_down_location_=1000;
  /*********************
   ** Wait for connection
   **********************/
...
void KeyOp::keyboardInputLoop()
{
  ...
  puts("---------------------------");
  puts("front/back arrows : linear velocity incr/decr.");
  puts("right/left arrows : angle velocity incr/decr.");
  puts("Z/C: move left/right.");
  puts("S/X: push up/down.");
...
void KeyOp::incrementLinearVelocity()
{
  if (power_status_)
  {
    ...
    ROS_INFO_STREAM("KeyOp: linear x  velocity incremented [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z <<  "|" <<up_down_location_ << "]");
  }
  else
  {
...
void KeyOp::decrementLinearVelocity()
{
  if (power_status_)
  {
    ...
    ROS_INFO_STREAM("KeyOp: linear x  velocity decremented [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z << "|" <<up_down_location_ << "]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
...
void KeyOp::incrementAngularVelocity()
{
  if (power_status_)
  {
    ...
    ROS_INFO_STREAM("KeyOp: angular velocity incremented [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z << "|" <<up_down_location_ <<"]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
...
void KeyOp::decrementAngularVelocity()
{
  if (power_status_)
  {
    ...
    ROS_INFO_STREAM("KeyOp: angular velocity decremented [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z << "|" <<up_down_location_ <<"]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
...
void KeyOp::moveLeft()
{
  if (power_status_)
  {
    if (cmd_->linear.y >= -linear_vel_max_)
    {
      cmd_->linear.y -= linear_vel_step_;
    }
    ROS_INFO_STREAM("KeyOp: linear y velocity decremented [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z <<  "|" <<up_down_location_ <<"]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
...
void KeyOp::pushUp()
{
  if (power_status_)
  {
    if (up_down_location_ < max_location_)
    {
      up_down_location_ += location_step_;
    }
    location_.up_down_location_=up_down_location_;
    up_down_publisher_.publish(location_);
    ROS_INFO_STREAM("KeyOp: push up location ["<< cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z << "|" << location_.up_down_location_ <<"]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
...
void KeyOp::pushDown()
{
  if (power_status_)
  {
    if (up_down_location_ > min_location_)
    {
      up_down_location_ -= location_step_;
    }
    location_.up_down_location_=up_down_location_;
    up_down_publisher_.publish(location_);
    ROS_INFO_STREAM("KeyOp: push down location [" << cmd_->linear.x << "|" << cmd_->linear.y << "|" << cmd_->angular.z <<  "|" <<location_.up_down_location_<<"]");
  }
  else
  {
    ROS_WARN_STREAM("KeyOp: motors are not yet powered up.");
